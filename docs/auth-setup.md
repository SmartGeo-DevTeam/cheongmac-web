...
## 4. 최초 회원 전환
OAuth 성공 시 사용자는 `PENDING` 상태로 생성됩니다. 이 상태는 홈페이지 회원 로그인이 아니라 `/join`에서 회원 전환을 완료하기 위한 임시 소셜 인증 세션입니다.

`/join`에서 본명/연락처/필수 약관을 확인하면 `membershipStatus=ACTIVE`와 `onboardingCompletedAt`이 함께 저장되고, 이 두 조건을 모두 만족해야 홈페이지 회원 로그인으로 인정합니다. 이메일은 소셜 계정에서 받은 값을 표시하며 임의 수정하지 않습니다.

회원 전환 전에 `/join` 흐름을 벗어나 일반 페이지나 `/signin`으로 이동하면 `/signout`을 거쳐 임시 세션을 강제로 종료합니다. 약관 확인에 필요한 `/terms`, `/privacy-policy`와 OAuth 완료 처리 `/auth/complete`만 임시 세션에서 허용합니다.

강제 로그아웃 라우트:
- `/signout`
- 필요하면 `?callbackURL=/원하는경로`를 붙여 로그아웃 후 이동 경로를 지정할 수 있습니다.
